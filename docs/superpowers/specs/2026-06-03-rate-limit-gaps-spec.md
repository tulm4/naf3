...
Current implementation decisions (locked by this plan):

- **AMF requests** use `perAmfPerSec` (second-based policy)
- **Subscriber-scoped traffic** (GPSI-based) uses `perGpsiPerMin` (minute-based policy)
- **Auth-context confirmation traffic** uses the subscriber-scoped minute policy (`perGpsiPerMin`) unless a dedicated limit is introduced later
- **`globalPerSec`** remains documented but is **not enforced** in this change unless explicitly wired during implementation

The exact enforcement points are defined so the same identity always maps to the same Redis key. See Task 3 (NSSAA) and Task 4 (AIW) for the specific handler wiring decisions.

### 6.3 Redis failure semantics

**Selected behavior: fail open.**

Rationale: These NSSAAF and AIW control-plane request paths preserve availability during transient Redis failures. The `rateLimit.failureMode` field is now validated and documented, but this implementation keeps runtime behavior fail-open on limiter errors across the affected handlers. When the limiter encounters a Redis error:

1. The error is logged with `scope` and hashed identity where applicable
2. A `result=error` decision is recorded in metrics
3. Request processing continues (fail-open)

A future change may wire `rateLimit.failureMode=closed` into handler behavior, but that is not part of this implementation.
